...
final class WordCounts {
    ...
    static Map<String, Integer> sort(Map<String, Integer> wordCounts, int popularWordCount) {
        ...
        return wordCounts.
                entrySet(). // create a Set from wordCounts Map
                stream(). // turn it to stream
                sorted(new WordCountComparator()). // sort the stream using WordCountComparator as sort Comparator
                limit(popularWordCount). // retrieve up to popularWordCount elements from the stream
                collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (key, value) -> key, LinkedHashMap::new)); // return a new Map of elements (limited in previous step) from the stream
